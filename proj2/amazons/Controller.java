...
final class Controller {
    ...
    void play() {
        ...
        while (_playing) {
            ...
            if (_winner == null) {
                _winner = _board.determineWinner();
                if (_winner != null) {
                    _reporter.reportNote("%s", _winner.toName() + " wins.");
                }
            }
        }
        if (_logFile != null) {
    ...
    /** MAT. Command "move". Added */
    private void doMove(Matcher mat) {
        _board.makeMove(mv(mat.group(0)));
    }

    /** MAT. Command "auto". Added */
    private void doAuto(Matcher mat) {
        String s = mat.group(1).toUpperCase();
