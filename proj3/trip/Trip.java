...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        int prevVertex = iter.next(), currVertex = iter.next();
        Road prevRoad = _map.getLabel(prevVertex,
                currVertex), currRoad;
        String currRoadName = "",
                prevRoadName = prevRoad.toString(), currDir = "",
                prevDir = prevRoad.direction().fullName(), currLocation = "";
        double prevDistance = prevRoad.length(), currDistance;
        while (iter.hasNext()) {
            prevVertex = currVertex;
            currVertex = iter.next();

            currLocation = _map.getLabel(currVertex).toString();
            currRoad = _map.getLabel(prevVertex, currVertex);

            currRoadName = currRoad.toString();
            currDir = currRoad.direction().fullName();
            currDistance = currRoad.length();

            if (currRoadName.equals(prevRoadName) && currDir.equals(prevDir)) {
                prevDistance += currDistance;
            } else {
                System.out.printf("%d. Take %s %s for %.1f miles.\n",
                        seq, prevRoadName, prevDir, prevDistance);

                prevRoad = currRoad;
                prevRoadName = prevRoad.toString();
                prevDistance = prevRoad.length();
                prevDir = prevRoad.direction().fullName();
        ...
        System.out.printf("%d. Take %s %s for %.1f miles to %s.\n",
                seq, currRoadName, currDir, prevDistance, currLocation);
    ...
    private void addRoad(String from, String name, double length,
        ...
        road = new Road(name, dir, length);
        roadReverse = new Road(name, reverseDir, length);

        _map.add(v0, v1, road);
        _map.add(v1, v0, roadReverse);
    }
